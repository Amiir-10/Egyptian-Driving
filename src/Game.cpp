...
Game::Game()
{
    currentState = MENU;
    isThirdPerson = true;
    cameraDistance = 8.0f; // Increased distance for larger 3D model
    cameraHeight = 4.0f;   // Raised camera for better view
    dayTime = 0.5f;        // Noon
...
void Game::setCamera()
{
    ...
    {
        // First person (driver's eye) - adjusted for larger 3D model
        float eyeX = carX + sin(carRot) * 1.5f; // Move forward past the car model
        float eyeZ = carZ + cos(carRot) * 1.5f;
        float lookX = carX + sin(carRot) * 15.0f;
        float lookZ = carZ + cos(carRot) * 15.0f;
        gluLookAt(eyeX, 1.2f, eyeZ, lookX, 1.5f, lookZ, 0.0f, 1.0f, 0.0f);
    }
}
...
void Game::handleSpecialInput(int key, int x, int y)
{
    if (currentState == LEVEL1 || currentState == LEVEL2)
    {
        ...
        {
        case GLUT_KEY_UP:
            playerCar.accelerate(true);
            break;
        case GLUT_KEY_DOWN:
            playerCar.brake(true);
            break;
        case GLUT_KEY_LEFT:
